...
sealed class MapUiState {
    ...
    data class MapContentUpdate(val bikes: List<Bike>?, val stations: List<Parking>?, val parkings: List<Parking>?, val slowZones: List<SlowZoneObject>?, val showMarkers: Boolean) : MapUiState()
...
//    data class ShowSlowZones(val slowZones: List<SlowZoneObject>, val showMarkers: Boolean) : MapUiState()
    ...
    data class ShowActiveRent(val activeRent: ActiveRent?, val show: Boolean) : MapUiState()
...
sealed class MapIntent {
    ...
    data class ActiveRentAction(val state: ActiveRentState/*val finishRent: Boolean, val isClosed: Boolean*/, val latitude: Double? = null, val longitude: Double? = null) : MapIntent()
